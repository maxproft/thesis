...
def nonlinde(C,D,oldstate):
        absolute = oldstate * oldstate.conjugate()
...
def linear(timestep,A,B,RealLength,LinList,pm,oldstate):
    #LinList = [np.exp((A-B*(3.14159265*(2*m-len(oldstate))/RealLength)**2)*timestep) for m in range(len(oldstate))]
    #fourier = np.fft.fft([old*(-1)**m for m,old in enumerate(oldstate)])
    #temp = [L*F for L,F in zip(LinList,fourier)]
    #invfft = np.fft.ifft(temp)
    #return [inv*(-1)**m for m,inv in enumerate(invfft)]

    #pm = np.array([(-1)**m for m in range(len(oldstate))])

    return np.fft.ifft(np.fft.fft(oldstate*pm)*LinList)*pm
...
def onestep(timestep,A,B,C,D,RealLength,LinList,pm,oldstate):
        #LinList = [np.exp((A-B*(np.pi*(2*m-len(oldstate))/RealLength)**2)*timestep) for m in range(len(oldstate))]
        
        #This line is equivalent to the 3 4 steps below. 
        return nonlinear(timestep*0.5,C,D,linear(timestep,A,B,RealLength,LinList,pm,nonlinear(timestep*0.5,C,D,oldstate)))

        #nonlinEvolution = nonlinear(timestep*0.5,C,D,oldstate)
        #linEvolution = linear(timestep,A,B,RealLength,LinList,nonlinEvolution)
        #secondNonLinEvolution = nonlinear(timestep*0.5,C,D,linEvolution)
        #return secondNonLinEvolution

def onestepgenerator(timestep,A,B,C,D,RealLength,LinList,pm,oldstate):
        yield oldstate
        current_state = oldstate
        halftime = timestep*0.5
        while True:
                current_state = onestep(timestep,A,B,C,D,RealLength,LinList,pm,current_state)#nonlinear(halftime,C,D,linear(timestep,A,B,RealLength,LinList,pm,nonlinear(halftime,C,D,current_state)))
                yield current_state
...
def alltime(timestep,numtimesteps,A,B,C,D,RealLength,oldstate):
    allsteps = oldstate
    newstep = oldstate
    LinList = np.array([np.exp((A-B*(np.pi*(2*m-len(oldstate))/RealLength)**2)*timestep) for m in range(len(oldstate))])
    pm = np.array([(-1)**m for m in range(len(oldstate))])
    ...
    gen = onestepgenerator(timestep,A,B,C,D,RealLength,LinList,pm,oldstate)
    ...
    #    newstep = onestep(timestep,A,B,C,D,RealLength,LinList,pm,newstep)
    #    allsteps2 = np.vstack((allsteps,newstep))
    #t3 = time.time()

    #print("Method 1 took %f seconds" % (t2-t1))
    #print("Method 2 took %f seconds" % (t3-t2))
    #return allsteps2
